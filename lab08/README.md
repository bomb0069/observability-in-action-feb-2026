...
- รวมข้อมูลจาก Apache logs (flog) และ JSON logs (flog2) ใน **กราฟเดียวกัน**
- ใช้ ECS (Elastic Common Schema) ทำให้ field names เหมือนกัน
- Split series by application สำหรับเปรียบเทียบใน single visualization
- Single index pattern ครอบคลุมทั้ง 2 applications
...
- **Logstash**: Log processor ที่ parse logs และ **normalize เป็น ECS format**
  - Apache logs: ใช้ grok patterns พร้อม GeoIP และ User Agent parsing (ECS format)
  - JSON logs: แปลง field names เป็น ECS format (status → http.response.status_code, etc.)
- **Elasticsearch**: Search and analytics engine สำหรับเก็บ logs ใน unified index:
  - `unified-logs-*`: All logs with ECS field names และ `app` field สำหรับแยก application
- **Kibana**: Visualization platform สำหรับสร้าง charts, graphs และ dashboards

## Lab Objectives

1. เรียนรู้การ **normalize field names** เป็น ECS format
2. ใช้ Logstash ในการแปลง JSON logs ให้มี ECS field structure
3. รวม logs จาก multiple applications ใน **single index pattern**
4. สร้าง **unified visualizations** ที่แสดงข้อมูลจาก 2 sources ใน single chart
5. ใช้ split series aggregation ด้วย `app` field
6. เข้าใจความสำคัญของ field standardization ใน multi-application monitoring
7. **สร้าง unified dashboard** ที่มี visualizations รวมข้อมูลทั้ง 2 applications
8. **Automatic Kibana index pattern creation** - สร้างอัตโนมัติผ่าน API
9. **สร้าง unified dashboard** ที่แสดงข้อมูลจากทั้ง 2 sources ในที่เดียว
10. เปรียบเทียบและวิเคราะห์ logs จาก 2 applications (flog มี 1 log/sec, flog2 มี 2 logs/sec)
...
✓ Index pattern created: flog2-logs-*
✓ Successfully imported saved objects!
Setup complete!
```
...
All of this happens automatically when you run `docker-compose up -d`! 🎉
...
3. You should see:
   - ✅ `flog-logs-*` (Apache logs) - set as default
   - ✅ `flog2-logs-*` (JSON logs)
